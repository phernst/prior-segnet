...
class FDKConvNet(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.network = UNet(depth=5, wf=5)
    ...
    def forward(self, *args, **kwargs):
        net_in = args[0][:, :1]
        return net_in + self.network(net_in)

    def training_step(self, *args, **kwargs):
        batch = args[0]
        batch_in, gt_reco, _ = batch
        prediction = self(batch_in)
        loss = self.reco_loss(prediction, gt_reco)
        return {
            'loss': loss,
        }
    ...
    def validation_step(self, *args, **kwargs):
        ...
        prediction = self(batch_in)
        loss = self.reco_loss(prediction, gt_reco)
        accuracy = self.accuracy(prediction, gt_reco)
...
def main():
    ...
    hparams.run_name = 'FDKConvNet_aug'
